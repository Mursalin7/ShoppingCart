...
import {addProductToCart, decrementCartQty, incrementCartQty, removeProductToCart} from "../../actions";
...
const CartProduct = (
    ...
    const incOrdec = (e, type) => {
        const value = itemQuantity;

        if(type === 'inc' && value < 10) {
            setItemQuantity(itemQuantity + 1);
            dispatch(incrementCartQty(id));
        }


        if(type === 'desc' && value > 1) {
            setItemQuantity(itemQuantity - 1);
            dispatch(decrementCartQty(id));
        }
